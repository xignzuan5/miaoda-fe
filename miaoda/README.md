...
- `miaoda-fe/design-token-scaffold` 只保存通用脚手架、固定扩展、安装器和示例 Token 基线，不包含任何业务项目的真实页面或审计结果。
...
如果妙搭应用可以直接从 Git 仓库同步，也可以先导入项目分支，再确认以上文件已经出现在应用代码目录中。只导入通用脚手架而没有具体项目源码时，解析器无法得到该项目的真实壳层、字体、路由和页面模板。
...
如果登录返回 `app is pending approval` 或“应用待审批”，脚手架只打印当前步骤并停止，不会一次输出后续命令：
