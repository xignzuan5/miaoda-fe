...
如果用户尚未有本地项目副本，可从脚手架根目录运行 `npm run miaoda:init`。该命令把 CLI 配置、浏览器授权、应用核对、Git clone、脚手架安装和首次扫描编排在一起；它只在目标目录不存在或已确认是同一妙搭仓库时继续，不覆盖非目标目录，也不自动推送。再次接入其他妙搭应用时重新运行该命令并选择新的应用和目录即可。

`miaoda:init`、`miaoda:pull` 和 `miaoda:push` 是本地/CI 适配命令，不是妙搭 Agent 的隐式动作。Skill 可以理解自然语言后调用它们，但所有授权、网络错误、远端分叉和推送结果都必须保留步骤、退出码、脱敏错误和远端 SHA；不能把“命令启动”报告成“代码已同步”。
