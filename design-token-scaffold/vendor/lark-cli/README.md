...
这里是离线运行目录。脚手架会在系统命令不可用、网络安装连续失败后自动检查这里，并按当前平台/CPU 架构优先查找对应子目录。

当前仓库内置一份经过校验的 Windows x64 版本：`win32-x64/lark-cli.exe`（v1.0.95）。其他操作系统或 CPU 架构不会使用这份文件，需按下方规则另行放置匹配目录。
...
备用包只解决 CLI 的安装下载问题，不能绕过飞书账号授权、妙搭应用权限、内网白名单或 `miaoda-git.feishu.cn` 的网络访问。升级版本时应重新取得官方包、核对 SHA-256，并更新对应平台目录；不要直接覆盖现有二进制。
