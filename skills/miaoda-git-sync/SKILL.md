...
3. 如果 `apps +get` 或 `apps +git-credential-init` 返回 `need_user_authorization`、`token_missing` 或 `missing_scope`，只补做一次 `lark-cli auth login --domain apps`，完成浏览器授权后重试原命令。浏览器登录、飞书管理员授权、网络白名单和 Windows 软件安装权限不能被绕过。遇到失败必须保留步骤名、退出码、脱敏后的错误和下一步建议。
