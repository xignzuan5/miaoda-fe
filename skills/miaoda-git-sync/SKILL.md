...
2. `miaoda:init` 可以检查或按公司规范执行 `npm install -g @larksuite/cli` 安装 `lark-cli`，网络安装最多重试 3 次；仍失败时使用 `vendor/lark-cli/` 或 `LARK_CLI_OFFLINE_DIR` 指向的完整离线包。若 lark-cli 尚未配置应用凭证，必须绑定管理员提供的 CLI app_id/app_secret，不得自动创建个人应用。随后触发浏览器授权，核对应用访问权限，获取妙搭 Git 地址，拉取 `sprint/default`，安装设计系统脚手架，并询问用户是否立即执行首次扫描；用户选择跳过时，提示之后在目标项目根目录运行 `npm run project:init`。
3. 如果 `apps +get` 或 `apps +git-credential-init` 返回 `need_user_authorization`、`token_missing` 或 `missing_scope`，只补做一次 `lark-cli auth login --scope "spark:app:read spark:app:write"`，完成浏览器授权后重试原命令；仅执行无参数的 `auth login` 可能显示登录成功，但不包含妙搭所需的 `spark` 权限。“本次已成功授权”只表示本次请求实际授予的权限，不是历史权限总表；如果它显示“（空）”且 spark 权限在“未授予”中，必须停止并让管理员开通权限，不能继续重试。若返回 `app is pending approval`、`pending_approval` 或“应用待审批”，必须停止重试并按命令输出的五步清单处理：填写公司申请表并将 Channel 配置选“否”→等待管理员在机器人消息中反馈已审批的 CLI `app_id/app_secret`→使用 `lark-cli config init --app-id ... --app-secret-stdin` 绑定凭证→执行上述 spark scope 登录和 `auth status`→重新运行 `npm run miaoda:init`（输入原目标妙搭应用 ID，不是 CLI app_id），再按平台页面配置飞书 Channel 并通知管理员添加事件/回调。浏览器登录、飞书管理员授权、网络白名单和 Windows 软件安装权限不能被绕过。遇到失败必须保留步骤名、退出码、脱敏后的错误和下一步建议。
